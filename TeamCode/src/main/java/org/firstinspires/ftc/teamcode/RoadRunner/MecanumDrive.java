...
/**
 *  mecanum驱动控制器，用于控制mecanum驱动式机器人的运动和定位。
 * 
 * 此类实现了轨迹跟踪、转弯控制、姿态估计等功能，
 * 使用编码器和IMU进行定位，并支持轨迹规划和执行。
 * Mecanum驱动允许机器人进行全方位移动（前进、后退、横向移动、旋转）。
 */
@Config
public final class MecanumDrive {
    /**
     * mecanum驱动参数配置类，包含所有驱动和控制相关的参数。
     * 
    ...
    public static class Params {
         ...
         */
        public double trackWidthTicks = 5862;

        /**
         * 静态摩擦力补偿系数，用于电机前馈控制。
         */
        public double kS = 1.3894152257076;
        
        /**
         * 速度增益系数，用于电机前馈控制。
         */
        public double kV = 0.0002512111623;
        ...
        public PoseVelocity2d update() {
            ...
            Twist2dDual<Time> twist = kinematics.forward(new MecanumKinematics.WheelIncrements<>
                    ...
                    new DualNum<Time>(new double[]{
                            (leftFrontPosVel.position - lastLeftFrontPos),  // 左前轮位移
                            leftFrontPosVel.velocity,                     // 左前轮速度
                    }).times(PARAMS.inPerTick),
                    new DualNum<Time>(new double[]{
                            (leftBackPosVel.position - lastLeftBackPos),   // 左后轮位移
                            leftBackPosVel.velocity,                      // 左后轮速度
                    }).times(PARAMS.inPerTick),
                    new DualNum<Time>(new double[]{
                            (rightBackPosVel.position - lastRightBackPos), // 右后轮位移
                            rightBackPosVel.velocity,                     // 右后轮速度
                    }).times(PARAMS.inPerTick),
                    new DualNum<Time>(new double[]{
                            (rightFrontPosVel.position - lastRightFrontPos), // 右前轮位移
                            rightFrontPosVel.velocity,                      // 右前轮速度
                    }).times(PARAMS.inPerTick)
                    )
            );
            ...
            pose = pose.plus(new Twist2d(
                    twist.line.value(),  // 线速度（来自编码器）
                    headingDelta         // 角速度（来自IMU）
            ));
